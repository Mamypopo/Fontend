        ...
        <div class="col-md-6">
          <div class="profile-head">
            ...
            <ul class="nav nav-tabs" id="myTab" role="tablist">
              <li class="nav-item">
                <button class="nav-link a active" id="home-tab" data-bs-toggle="tab" data-bs-target="#home-tab-pane"
                  type="button" role="tab" aria-controls="home-tab-pane" aria-selected="true">Profile</button>
              </li>
              <li class="nav-item" v-if="getUser.role === 'student'">
                <button class="nav-link" id="hours-tab" data-bs-toggle="tab" data-bs-target="#hours-tab-pane"
                  type="button" role="tab" aria-controls="hours-tab-pane" aria-selected="false">Hours</button>
              </li>
                  ...
                  <Form @submit="editInfor">
                    ...
                    <div class="row">
                      <div class="col-8 col-sm-4">
                        ...
                        <input type="text" class="form-control" name="studentId" v-model="studentId"
        ...
        <div class="col-md-8">
          <div class="tab-content profile-tab" id="myTabContent">
            ...
            <div class="tab-pane fade" id="hours-tab-pane" role="tabpanel" aria-labelledby="hours-tab" tabindex="0">
              <div class=" justify-content-center d-flex" id="printable-content">
                <table class="table table-bordered" style="width: 800px;">
                  ...
                  <tbody>
                    <tr v-for="activity, index in activities" :key="index">
                      <td>{{ new Date(activity.date).toLocaleDateString('th', { dateStyle: 'medium' }) }}</td>
                      <td>{{ activity.location }}</td>
                      <td>{{ activity.name }}</td>
                      <td>{{ Number(activity.hourGain) }}</td>
                      <td>{{ activity.updateStatusBy }}</td>
                      <td>{{ showStatus(activity.approveStatus) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <p class="text-danger text-center">*ทำกิจกรรมจิตอาสาของคณะ เทอมละ 4 ชั่วโมง</p>
              <div class="justify-content-center d-flex">
                <button @click="printContent" type="button" class="btn btn-primary ms-3">พิมพ์</button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async printContent() {
      try {
        const { data } = await axios.get('http://localhost:8000/api/v1/activities/getStudentActivityDoc', {
          headers: {
    ...
    async editInfor() {
      try {
        ...
        if (isConfirmed) {
          const response = await axios.post('http://localhost:8000/api/v1/students/updateStudent', formData, {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`
            }
    ...
    async getActivity() {
      try {
        const { data } = await axios.get('http://localhost:8000/api/v1/activities/getStudentActivity', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        })
